...
jest.mock('expo-crypto', () => ({
  getRandomBytes: (length: number) => {
    ...
    for (let index = 0; index < length; index += 1) bytes[index] = (index * 31 + 17) % 256;
    return bytes;
  },
}));
...
import { decryptData, deriveKeys, encryptData, hashPin, validatePin, verifyPin } from '../lib/encryption';
...
describe('vault cryptography', () => {
  ...
  it('round-trips authenticated encrypted data with associated data', async () => {
    ...
    const encrypted = await encryptData('license-ABC-123', databaseKey, 'product-1');
    expect(encrypted.version).toBe(2);
    expect(await decryptData(encrypted, databaseKey, 'product-1')).toBe('license-ABC-123');
  });

  it('rejects altered ciphertext and incorrect associated data', async () => {
    const { databaseKey } = await deriveKeys(masterKey);
    const encrypted = await encryptData('private value', databaseKey, 'product-1');
    await expect(decryptData({ ...encrypted, tag: `${encrypted.tag.slice(0, -2)}ff` }, databaseKey, 'product-1')).rejects.toThrow('Unable to decrypt vault record');
    await expect(decryptData(encrypted, databaseKey, 'product-2')).rejects.toThrow('Unable to decrypt vault record');
  });
